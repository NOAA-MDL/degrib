 ...
 *****************************************************************************
 */
static int GRIB2SectToBuffer (FILE *fp, uInt4 gribLen, sChar *sect,
                              uInt4 *secLen, uInt4 *buffLen, char **buff)
{
   ...
   if (fread (buffer, sizeof (char), *secLen - sizeof (sInt4), fp) !=
       *secLen - sizeof (sInt4)) {
      if (*sect != -1) {
         errSprintf ("ERROR: Ran out of file in Section %d\n", *sect);
      } else {
 ...
 *****************************************************************************
 */
static int GRIB2SectJump (FILE *fp, sInt4 gribLen, sChar *sect, uInt4 *secLen)
{
   ...
   if (fread (&sectNum, sizeof (char), 1, fp) != 1) {
      if (*sect != -1) {
         errSprintf ("ERROR: Ran out of file in Section %d\n", *sect);
      } else {
         errSprintf ("ERROR: Ran out of file in GRIB2SectSkip\n");
 ...
 *****************************************************************************
 */
static int GRIB2Inventory2to7 (sChar sectNum, FILE *fp, sInt4 gribLen,
 ...
 *****************************************************************************
 */
int GRIB2Inventory (char *filename, inventoryType **Inv, uInt4 *LenInv,
