 ...
 *
 * PURPOSE
 *   To do an "fread" into a 4 byte long int, but in a reverse manner with not
 ...
 *****************************************************************************
 */
size_t FREAD_ODDINT_BIG (long int *dst, unsigned char len, FILE * fp)
{
   *dst = 0;
#ifdef LITTLE_ENDIAN
 ...
 *****************************************************************************
 */
size_t FREAD_ODDINT_LIT (long int *dst, unsigned char len, FILE * fp)
{
   *dst = 0;
#ifdef LITTLE_ENDIAN
 ...
 *
 * PURPOSE
 *   To do an "fwrite" from a 4 byte long int, but in a reverse manner with
 * not necessarily all 4 bytes.  It writes big endian data to disk.
 *
 * ARGUMENTS
 ...
 *****************************************************************************
 */
size_t FWRITE_ODDINT_BIG (long int *src, unsigned char len, FILE * fp)
{
#ifdef LITTLE_ENDIAN
 ...
 * PURPOSE
 *   To do an "fwrite" from a 4 byte long int, but in a reverse manner with
 * not necessarily all 4 bytes.  It writes little endian data to disk.
 *
 * ARGUMENTS
 * src = Where to read the data from. (Output)
 * len = The number of bytes to read. (<= 4) (Input)
 ...
 *****************************************************************************
 */
size_t FWRITE_ODDINT_LIT (long int *src, unsigned char len, FILE * fp)
{
#ifdef LITTLE_ENDIAN
 ...
 *****************************************************************************
 */
int fileBitRead (void *Dst, size_t dstLen, unsigned short int num_bits,
                 FILE * fp, unsigned char *gbuf, int *gbufLoc)
{
   static unsigned char BitRay[] = { 0, 1, 3, 7, 15, 31, 63, 127, 255 };
   register unsigned char buf_loc, buf, *ptr;
   ...
   dst_loc = (unsigned char) ((num_bits % 8) + 1);
   buf_loc = *gbufLoc;
   buf = *gbuf;
...
#ifdef LITTLE_ENDIAN
      *(ptr--) |= (unsigned char) ((buf & BitRay[buf_loc]) >>
                                   (buf_loc - dst_loc));
#else
      *(ptr++) |= (unsigned char) ((buf & BitRay[buf_loc]) >>
                                   (buf_loc - dst_loc));
...
#endif
      if (buf_loc != 0) {
         *ptr |= (unsigned char) ((buf & BitRay[buf_loc]) << (8 - buf_loc));
      }
      /* buf_loc is now 0. so we need more data. */
      if ((c = fgetc (fp)) == EOF) {
         *gbufLoc = buf_loc;
 ...
 *****************************************************************************
 */
int fileBitWrite (void *Src, size_t srcLen, unsigned short int numBits,
                  FILE * fp, unsigned char *pbuf, int *pbufLoc)
{
   unsigned char buf_loc, buf, *ptr;
   unsigned char *src = Src;
   size_t num_bytes;
   unsigned char src_loc;
   ...
   /* num_bits was modified earlier. */
   src_loc = (unsigned char) ((numBits % 8) + 1);
   buf_loc = *pbufLoc;
   buf = *pbuf;
   ...
   if (buf_loc >= src_loc) {
...
#ifdef LITTLE_ENDIAN
      buf |= (unsigned char) ((*(ptr--) & ((1 << src_loc) - 1)) <<
                              (buf_loc - src_loc));
#else
      buf |= (unsigned char) ((*(ptr++) & ((1 << src_loc) - 1)) <<
                              (buf_loc - src_loc));
      ...
      buf_loc += (unsigned char) (8 - src_loc);
#ifdef LITTLE_ENDIAN
      buf |= (unsigned char) (*(ptr--) << buf_loc);
#else
      buf |= (unsigned char) (*(ptr++) << buf_loc);
#endif
   }
   ...
   while (ptr < src + srcLen) {
      ...
      if (buf_loc == 0) {
         ...
         buf |= (unsigned char) ((*ptr) >> (8 - buf_loc));
         /* buf_loc is now 0, so we write it out. */
         if (fputc ((int) buf, fp) == EOF) {
            *pbufLoc = buf_loc;
            *pbuf = buf;
