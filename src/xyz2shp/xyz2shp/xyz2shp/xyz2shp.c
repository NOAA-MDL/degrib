   ...
   while (reallocFGets (&line, &lineLen, fp) != 0) {
      ...
      if (lineNum == 0) {
         ...
         *numStat = *numStat + 1;
         ...
         myAssert (*numCol >= 3);
         (*Stat)[*numStat - 1].ptr = (char **)
            malloc ((*numCol - 2) * sizeof (char *));

         (*Stat)[*numStat - 1].numPtr = *numCol - 2;
         for (i = 0; i < lineArgc; i++) {
            strTrim (lineArgv[i]);
            ...
            if (i < 2) {
               ...
               (*Stat)[*numStat - 1].ptr[i - 2] = (char *)
                  malloc ((fldLen + 1) * sizeof (char));

               strcpy ((*Stat)[*numStat - 1].ptr[i - 2], lineArgv[i]);
            }
         }
      }
      for (i = 0; i < lineArgc; i++) {
...
int main (int argc, char **argv)
{
   ...
   if (ReadXYZ (argv[1], &Stat, &numStat, &Col, &numCol) != 0) {
