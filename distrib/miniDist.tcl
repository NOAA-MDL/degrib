...
if {$argc == 1} {
  set argv2 [clock seconds]
} else {
  set argv2 [clock scan [lindex $argv 1]]
}
...
if {$ray(Version) < $argv1} {
  set ray(Version) $argv1
...
} elseif {$ray(Version) == $argv1} {
} else {
  puts "release $ray(Version) is > $argv1"
  puts "Resetting release version"
  set ray(Version) $argv1
}
set date [clock format $argv2 -format "%m/%d/%Y"]
set date2 [clock format $argv2 -format "%Y%m%d"]
...
#####
puts [exec bash miniPack.tcl src.txt]

#####
# 7) Update some of the pages on the web site.
#####
set webDir [file dirname [file dirname $src_dir]]
set date2 [clock format $argv2 -format "%Y%m%d"]
# exec mv [file join $webDir degrib-src.tar.gz] \
#         [file join $webDir degrib-src-$date2.tar.gz]
